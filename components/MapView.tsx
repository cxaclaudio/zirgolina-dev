...
/**
 * Cor do preço por tipo de combustível (texto da DGEG já normalizado):
 *   GPL Auto                          → azul  #2563eb
 *   Gasóleo simples/especial/colorido → preto #1a1a1a
 *   Gasolina simples/especial 95/98   → verde #16a34a
 */
function corPorTipoCombustivel(tipo: string): string {
  const t = normalizeName(tipo);
  if (t.startsWith("gpl") || t.includes("petroleo liquefeito")) return "#2563eb";
  if (t.startsWith("gasoleo")) return "#1a1a1a";
  if (t.startsWith("gasolina")) return "#16a34a";
  // fallback parcial
        ...
        postos.forEach((posto) => {
          ...
          const combsHtml =
            posto.combustiveis.map((c: any) => {
              ...
              // Usar hex literal direto — sem variáveis CSS para evitar cascade
              ...
              return `
                <div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;font-size:0.72rem;margin-bottom:2px">
          ...
          const direcUrl = posto.lat && posto.lng
          ...
          const marker = L.marker([posto.lat, posto.lng], { icon }).bindPopup(
            `<div style="min-width:190px;font-family:sans-serif;all:initial;display:block">
  <p style="all:unset;display:block;font-weight:700;margin:0 0 2px;font-family:sans-serif">
    <span style="color:${marcaCor}">${posto.marca}</span>
    <span style="color:#aaa;margin:0 0.3rem">|</span>
    <span style="color:#222">${posto.nome}</span>
  </p>
  <p style="all:unset;display:block;font-size:0.72rem;color:#888;margin:0 0 6px;font-family:sans-serif">${posto.localidade}</p>
  ${combsHtml}
  ${descontoBadge}
  <a href="${direcUrl}" target="_blank" rel="noopener noreferrer"
    style="all:unset;display:inline-flex;align-items:center;gap:0.3rem;margin-top:8px;padding:4px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:0.67rem;font-weight:500;color:#555;text-decoration:none;cursor:pointer;font-family:sans-serif">
